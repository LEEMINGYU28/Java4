...
public class BufferedTest {
	...
	public static void test() {
		...
		try {
			...
			System.out.println(path + "다운로드 (1).jpg");
			FileInputStream fis = new FileInputStream(path + "다운로드 (1).jpg");
			FileOutputStream fos = new FileOutputStream(path + "output1.jpg");

			FileInputStream fis2 = new FileInputStream(path + "다운로드 (1).jpg");
			FileOutputStream fos2 = new FileOutputStream(path + "output2.jpg");

			BufferedInputStream bis = new BufferedInputStream(fis2);
			BufferedOutputStream bos = new BufferedOutputStream(fos2);
			// 이미지가 느리다면 Buffered 사용
