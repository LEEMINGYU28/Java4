...
//		// 어떠한 유저(user)가 자산의 글(board)를 수정한다.
...
//			conn.rollback();
			tm.rollback(status); //status 현재상태에 대해서 넣는다.
			throw e;
		} finally {
//			DataSourceUtils.releaseConnection(conn, dataSource);
//			TransactionSynchronizationManager.unbindResource(dataSource);
//			TransactionSynchronizationManager.clearSynchronization(); // 동기화 했던 모든 정보를 없애줘라
		}
	}
