...
import c231020.board.BoardTestDAO;
...
public class BoardTest {
	...
	public void initialize() throws Exception {
		...
		board.setTitle("확인중1");
		board.setUserId(1);
		board.setContent("확인중 내용1");
		boardDAO.add(board);
		board = new BoardBean();
		board.setTitle("확인중2");
		board.setUserId(1);
		board.setContent("확인중 내용2");
		boardDAO.add(board);
		board = new BoardBean();
		board.setTitle("확인중3");
		board.setUserId(1);
		board.setContent("확인중 내용3");
		boardDAO.add(board);
	...
	public void get() {
		...
		assertThat(board.getContent(), is("확인중 내용1"));
	...
	public void getAll() {
		...
		for(int i = 0; i < list.size();++i) {
			...
			assertThat(list.get(i).getContent(), is("확인중 내용"+ (i + 1)));
		};		
	}
